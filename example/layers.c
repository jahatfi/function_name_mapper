...
//Verify preprocessing is performed
#define PI 3.14
int global_int = 6;
...
char * return_tainted_buff(){
    ...
    if (!buff) return NULL;
    printf("Enter some text to execute\n");
    gets(buff);
    return buff;
}
...
bool layer_3_a(char * buff, int count, bool flag){
   ...
   if(count > 128){
      if(flag){
          printf("A tragedy indeed!");
          printf("Flag must be false.\n");
          return false;
      }
      else if (count > 256){
          // Remove these statements in the output by toggling "compress-paths"
          printf("A");
          printf("B");
          printf("C");
...
void main (int argc, char * argv[]){
    ...
    // Lines like the one below mess up some parsers
    //printf("{{{{{;,;,;}\"{}[}}{]{{");

    enum year{Jan, Feb, Mar, Jun=6, Jul};
    //printf("Jan is month #: %d, Jun is #%d, and July is #%d\n", Jan, Jun, Jul);
